...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #print(embed_size,self.hidden_size,self.dropout_rate,len(self.vocab.src),len(self.vocab.tgt),)
...
#        print(source_lengths)
        ...
        X_1 = pack_padded_sequence(input=X, lengths=source_lengths, batch_first=False, enforce_sorted=True)
        output_1, (h_n_1, c_n_1) = self.encoder(X_1)
        enc_hiddens, lens_unpacked = pad_packed_sequence(output_1, batch_first=True)
        #enc_hiddens.transpose_(0, 1)
        hm_h1 = torch.cat((h_n_1[0],h_n_1[-1]), dim=1,)
        cm_c1 = torch.cat((c_n_1[0],c_n_1[-1]), dim=1,)
        h_0 = self.h_projection(hm_h1)
        c_0 = self.c_projection(cm_c1)
    ...
    #        print(Y_t.size())
            Y_t_1= torch.squeeze(input=Y_t, dim = 0)
   #         print(o_prev.size())
            Ybar_t = torch.cat((Y_t_1,o_prev), dim=1,)
   #         print(Ybar_t.size())
      ...
      #      b,h = dec_state
        ...
        Y_2 = torch.stack(combined_outputs,dim=0,)
     #   print('Y_2',Y_2.size())
        return Y_2
        ### END CODE HERE
   ...
   #     print('dec',dec_hidden.size(), dec_cell.size())
        dec_hidden_b = torch.unsqueeze(dec_hidden, 1)
   #     print('dec',dec_hidden_b.size())
        enc_hiddens_proj_b = torch.transpose(enc_hiddens_proj,1,2)
    #    print(enc_hiddens_proj_b.size())
        e_t_b = torch.bmm(dec_hidden_b,enc_hiddens_proj_b)
        e_t = torch.squeeze(e_t_b,dim=1)
     #   print('et', e_t.size())
        ### END CODE HERE
        ...
        alpha_t_b = torch.unsqueeze(alpha_t,1)
      #  print(alpha_t_b.size(),enc_hiddens.size())
        a_t_b = torch.bmm(alpha_t_b,enc_hiddens)
      #  print(a_t_b.size())
        a_t = torch.squeeze(a_t_b,1)
       # print(a_t.size())
        U_t = torch.cat((dec_hidden,a_t),dim=1)
        #print(U_t.size())
        V_t = self.combined_output_projection(U_t)
       # print(V_t.size())
        O_1_t = torch.tanh(V_t)
        O_t = self.dropout(O_1_t)
       # print(O_t.size())
         #           self.h_projection = nn.Linear(in_features=2*self.hidden_size, out_features=self.hidden_size, bias=False)
          #          self.c_projection = nn.Linear(in_features=2*self.hidden_size, out_features=self.hidden_size, bias=False)
         #           self.att_projection = nn.Linear(in_features=2*self.hidden_size, out_features=self.hidden_size, bias=False)
        ...
        #            self.target_vocab_projection = nn.Linear(in_features=self.hidden_size, out_features=len(self.vocab.tgt), bias=False)
        #            self.dropout = nn.Dropout(p=self.dropout_rate)
              #        Y_1 = pack_padded_sequence(input=Y, lengths=source_lengths, batch_first=False, enforce_sorted=True)
              #       output_1, (h_n_1, c_n_1) = self.encoder(Y_1)
              #       enc_hiddens, lens_unpacked = pad_packed_sequence(output_1, batch_first=True)
        ### END CODE HERE
